...
namespace Library
{
    public partial class ProfileLibrarian : Form
    {
        ...
        public ProfileLibrarian(int readID)
        {
            InitializeComponent();
            rID = readID;
                    ...
                    if (textBoxNewPassword.Text != textBoxNowPassword.Text)
                    {
                        if (textBoxNewPassword.Text == textBoxAccess.Text)
                        {
                            ...
                            if (dialog == MessageBox.Show("Вы уверены, что хотите поменять пароль?", "Предупреждение", MessageBoxButtons.OKCancel, MessageBoxIcon.Question))
                            {
                                ...
                                conn.SaveChanges();
                                MessageBox.Show("Пароль успешно изменён", "Информация", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                                this.Close();
                            }
                        }
                        else
